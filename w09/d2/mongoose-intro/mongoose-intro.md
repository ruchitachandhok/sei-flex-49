...
Alternately, instead of using the new Movie() + save() methods, mongoose also provides you with the create() method, so you could also do this:
```js
function create(req, res) {
  // convert nowShowing's checkbox of nothing or "on" to boolean
  req.body.nowShowing = !!req.body.nowShowing;
...
Note also that although here we are directly storing the req.body object into our database, we could instead have decided to do something like this:
```js
...
that matches your Movie schema and model that we specified above in our models/ file:
```js
{
  title: String,
  releaseYear: Number,
...
> Note: we use Movie.findById(req.params.id, callback) to retrieve one specific movie from the DB.
