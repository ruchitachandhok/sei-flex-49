...
- Functions optionally accept arguments as input and return a single piece of data, for example:
	...
	const points = getPointsScored(5); // might return 10 points
	const points = getPointsScored(20); // might return 100 points.
	```
...
Tomororw, we will look at a version of the `getRandomInt` function that is defined to accept two arguments (inputs). These arguments should be numbers, otherwise the function as written won't work as expected, but for today we will use a simpler version that simply accepts no arguments --- no inputs!
...
### 4. Parameters/Arguments (Tomorrow!)
