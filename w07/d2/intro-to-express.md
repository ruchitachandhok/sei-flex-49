...
- In the spirit of saving time, copy/paste the following inside of `todo-db.js`, then we'll review the code:
	...
	module.exports = {
	  getAll: getAll, // yes - we can export functions!
	};
	```
...
- To access our To Do "database", we need to `require` it inside of **server.js**:
	...
	const todoFile = require('./data/todo-db');
	```
...
- Add this new route:
	...
	app.get('/todos', function(req, res) {
	  ...
	  res.render('todo-index.ejs', todosObject);
	});
	```
...
- The `<% %>` EJS tags are for executing JavaScript statements such as control flow.
